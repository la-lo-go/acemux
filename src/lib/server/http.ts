...
export function engineBase(): string {
  return (process.env.ACESTREAM_BASE || 'http://localhost:6878').replace(/\/+$/, '')
}
...
/**
 * Authorizes a request against API_TOKEN. When no token is configured the
...
/**
 * Base URL emitted in the M3U. Uses PUBLIC_URL when configured, otherwise
